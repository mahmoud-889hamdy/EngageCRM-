...
const PricingCard: FC<PricingCardProps> = ({ annuallyPrice, description, features, monthlyPrice, title, glowPosition, isMostPopular, mode }) => {
    return <article className={`relative w-full py-6 px-4 flex flex-col space-y-4 lg:space-y-6 ${isMostPopular ? "border-2 border-sky-700 bg-slate-800" : "border border-slate-700"} rounded-lg overflow-hidden`}>
